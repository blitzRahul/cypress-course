...
describe('Accordion.cy.jsx', () => {
  it('items accordion', () => {
    ...
    //checking to see if the accordion has 8 elements
    cy.getDataTest("accordion-wrapper").within(()=>{
      cy.get("[data-test^=ac-item]").should("have.length",8);
    });

    //now checking if the text is not visible before clicking
    cy.contains(/Your tests will exist in a describe/i).should("not.be.visible");
    cy.getDataTest("ac-item-1").click();
